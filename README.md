...
Используется простой `TypeConverter`, который в текущей реализации фактически пробрасывает типы без изменения. Это соответствует задаче, так как здесь важно не менять типовую систему, а показать правильную инфраструктуру `Dialect Conversion`.
